...
const SystemsInventory = () => {
  ...
  const handleAddSystem = () => {
    ...
    const system = {
      ...
      monthlyCost: newSystem.monthlyCost || 0,
      supportCost: newSystem.supportCost || 0,
                                    ...
                                    <td className="p-3 border-r">
                                      {isSelected && (
                                        <Input
                                          type="text"
                                          placeholder="R$ 0,00"
                                          value={systemData.supportCost ? formatCurrency(systemData.supportCost) : ''}
                                          onChange={(e) => {
                                            const numericValue = parseCurrencyInput(e.target.value);
                                            const updatedSystems = systems.map(s => 
                                              s.name === system.name 
                                                ? { ...s, supportCost: numericValue }
                                                : s
                                            );
                                            setSystems(updatedSystems);
                                          }}
                                    ...
                                    <td className="p-3">
                                      {isSelected && (
                                        <Input
                                          type="text"
                                          placeholder="R$ 0,00"
                                          value={systemData.infrastructureCost ? formatCurrency(systemData.infrastructureCost) : ''}
                                          onChange={(e) => {
                                            const numericValue = parseCurrencyInput(e.target.value);
                                            const updatedSystems = systems.map(s => 
                                              s.name === system.name 
                                                ? { ...s, infrastructureCost: numericValue }
                                                : s
                                            );
                                            setSystems(updatedSystems);
                                          }}
